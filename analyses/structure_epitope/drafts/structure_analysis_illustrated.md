...
to a per-predictor file for the SI cross-predictor comparison (SI Tables S7–S9), which anchors every comparison to Protenix. The *learned confidence scores* quoted in §3 and Fig 11 are, by contrast, the competition's own Boltz-2 values — the submitted ipSAE, Boltz-2 ipTM, and Boltz-2 binder pLDDT — so §3 pairs Protenix structural descriptors with Boltz-2 confidence scores; SI Table S8 gives every score on every model, where the confidence scores' backbone dependence is explicit. The
