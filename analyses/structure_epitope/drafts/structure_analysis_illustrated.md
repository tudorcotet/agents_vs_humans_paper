...
> **Illustrated version** — main-text figures embedded inline; prose is identical to [`structure_analysis.md`](structure_analysis.md). Supplementary figures S4–S6 are embedded in [`supplementary_information_illustrated.md`](supplementary_information_illustrated.md).
...
- **Interface size is the only interface *descriptor* that discriminates binders from non-binders — robustly, not most strongly.** Binders bury a larger interface (1,823 vs. 1,515 Å², AUROC 0.72, q=0.004), and the effect holds at AUROC 0.65–0.72 across all five predictors (including the no-MSA ESMFold2). It is *not* the best discriminator overall, however: the learned confidence scores match or beat it on the clean AF3-class models (Protenix ipTM 0.79, ipSAE 0.78 vs. buried area 0.72; mean pLDDT 0.71 about level; Chai higher still). Those scores are simply backbone-dependent — ipSAE falls to 0.65 on the Boltz-2 model used for selection — so interface size's advantage is *robustness* across models, not raw AUROC. Interface chemistry, secondary structure, and contact motifs add nothing beyond size — even interface H-bonds and salt bridges, re-scored with proper geometry (Rosetta HBondSet) on clash-verified relaxed structures, reduce to interface size (§2). 
- **Affinity — learned scores don't *rank* K_D among binders; physical/structural quantities and length do.** Discriminating binders from non-binders (§2) and ordering binders by affinity are different questions, and the *same* Protenix confidence scores that discriminate strongly (AUROC 0.71–0.79) do the first but not the second: on Protenix ipSAE ρ=0.29, ipTM 0.20 and mean pLDDT 0.28 are all non-significant, as are the language-model likelihoods (ESM-2, SaProt); the one learned score that clears FDR is the competition's own Boltz-2 ipSAE (the selection metric, ρ=0.40, agent-driven). The Protenix structural quantities — interface size (ρ=0.47), buried area (0.50), PRODIGY ΔG (0.52) — and binder length (0.65) do (pooled n=36). The human-vs-agent split is mostly statistical power, not real: interface size and buried area are underpowered — not absent — in the 12 agents, and length works in both cohorts; only PRODIGY (human-only) and ipSAE (an agent-only inversion) are genuinely cohort-specific (§3).
...
![Fig 8a-d](../figures/blender/sample_renders/fig08_partners.png)

***Fig 8a–d.*** *TREM2 binding partners in a common orientation — (a) our binder (design 17), (b) scFv-2 (6YYE), (c) scFv-4 (6Y6C), (d) phosphatidylserine (6B8O); each co-crystal rigid-body-superposed onto the design TREM2 frame. The antibodies engage the distal β-A/F/G face; our binder and the lipid occupy the opposite apical face.*
...
![Fig 8e](../figures/fig08e_epitope_landscape.png)

***Fig 8e.*** *Epitope landscape of the TREM2 IgSF domain — per-residue binder contact frequency (binders vs non-binders; consensus footprint), with the apical hydrophobic tip lighting up near-universally.*

![Fig 10](../figures/blender/sample_renders/fig10_gallery.png)

***Fig 10.*** *Complex gallery on the shared TREM2 frame — (a) the top designs in leaderboard (ipSAE) order, (b) the best design's interface with its EvoEF2 hotspot side chains, (c) all 37 hit binders superposed, converging on the apical face.*
...
Interface size is, however, not the *best* binder/non-binder discriminator overall — only the best *interface descriptor*. The competition's learned confidence scores match or beat it on the two clean AF3-class models: on Protenix, ipTM (AUROC 0.79) and ipSAE (0.78) exceed buried area (0.72) and mean pLDDT (0.71) is about level; on Chai all three exceed it (SI Table S8, visualized in **Fig S8**). What sets interface size apart is *robustness*, not raw AUROC — the learned scores are strongly backbone-dependent (ipSAE 0.78 on Protenix/Chai but only 0.65 on the Boltz-2 model the organizers used to select the top 100; ipTM 0.79→0.60), whereas interface size holds at 0.65–0.72 on all five predictors, including the single-sequence, no-MSA ESMFold2. This is the same instability that puts ~14% of the Boltz-2-ipSAE top-100 outside a Protenix-ipSAE top-100 (Kendall τ=0.52, Fig S4b), seen from the discrimination side rather than the ranking side: interface size reads the same regardless of which model drew the interface.

![Fig 9](../figures/fig09_interface_features.png)

***Fig 9.*** *Interface size — not epitope or chemistry — separates binders from non-binders (P_expressed, n=89): binders bury a larger interface (AUROC 0.72), an effect that reproduces across all five predictors.*
...
Furthermore, we classified design outcomes using the Failure taxonomy (Bennett et al., 2023), yielding a three-way decomposition: **11 Type 0 expression failures, 17 Type I folding failures, 35 Type II interface failures**, and 37 active binders. Folding failure was defined as a binder Cα-RMSD >3 Å between the free-monomer ESMFold prediction and the binder conformation in the predicted complex; interface failures retained the predicted fold but did not bind.Among the 52 expressed non-binders, interface failures outnumbered folding failures by approximately 2:1. Active binders also showed greater monomer–complex agreement than non-binders (median Cα-RMSD 0.63 vs. 1.27 Å). Together with the interface-size analysis, these results suggest that **insufficient interface formation was a more common limitation than gross folding failure**.

## 3. Affinity: computational predictors and literature context

For affinity (P_kd, n=36), we find that **the learned scores do not rank K_D among binders** — a separate question from the binder/non-binder discrimination of §2, and one they fail even though several of them discriminate well there. On Protenix — the section's primary model, so this is apples-to-apples with §2 — none of the learned scores ranks affinity: ipSAE ρ=0.29, ipTM 0.20 and mean pLDDT 0.28 all have CIs spanning zero, as do the language-model likelihoods (ESM-2 0.09, SaProt 0.06). The *same* Protenix confidence scores that discriminate binders from non-binders strongly (§2: AUROC 0.71–0.79) thus fail to order those binders by K_D. The one learned score that clears FDR is the competition's own **Boltz-2 ipSAE** (the submitted top-100 selection metric, ρ=0.40, q=0.10) — and it only just does: that clearance is boundary-fragile, contingent on the exact makeup of the confidence-score FDR family (one more metric entering the tier would return it to q>0.10). Its signal, moreover, comes *entirely from the agent cohort* (0.66 vs. human 0.30, ns). **The Protenix structural quantities, on the other hand, do correlate with experimental affinity measurements**: interface size (0.47), buried area (0.50), and PRODIGY ΔG (0.52) clear FDR, and **binder length is the strongest correlate** (0.65), though interface burial keeps its signal after controlling for length (partial ρ≈0.51), so it is not merely a length proxy. Controlling instead for **buried area** collapses the structural correlates to a single interface-size axis: PRODIGY ΔG falls from 0.52 to 0.22 and interface-residue count from 0.47 to 0.16 (both n.s.), and no interface-chemistry descriptor — H-bonds, salt bridges, hydrophobic fraction — retains any ranking signal once BSA is removed. The PRODIGY/BSA relationship is symmetric (BSA controlling for PRODIGY is equally small, 0.15 n.s.; the two correlate at ρ=0.82), so this shows **PRODIGY and buried area are one signal, not two** — an equivalence, not a reduction of PRODIGY to BSA; reading that shared axis *as* interface size rests on PRODIGY being a contact-count model, a mechanistic argument rather than a statistical one. Binder length, by contrast, is nearly independent of interface size (BSA–length ρ=0.19, and length's correlation is unchanged controlling for BSA), so **binder length and interface size are two largely separate affinity signals** (SI `partial_correlation_bsa.md`), not one confound. Because interface area and PRODIGY are read off the predicted complex, this is a structural property of the model tracking affinity, not sequence-level K_D prediction.

![Fig 11](../figures/fig11_metrics_vs_affinity.png)

***Fig 11.*** *Structural interface size & binder length track affinity while learned confidence scores largely do not (P_kd, n=36; human vs agent). Buried area, interface size, and PRODIGY ΔG correlate with measured K_D; the two right-hand panels show the same ipSAE giving different answers by backbone — Protenix (ρ=0.29, n.s.) vs the competition's Boltz-2 selection metric (ρ=0.40).*


Regarding the human-vs-agent comparison, we concluded that the contrast is a **decomposition, not a within-human story**. For interface size and buried area the 12 agents match the humans' direction but miss significance (ρ 0.36–0.41, n.s.) purely from n=12 underpower, whereas **PRODIGY is genuinely human-only** (agent ρ=0.04). Finally, the one agent-specific ranking signal is carried by the **competition's Boltz-2 ipSAE** (agent ρ=0.66 vs. human 0.30, ns); the Protenix ipSAE leans the same way but weakly and clears nothing in either cohort (agent 0.47, human 0.28) (SI Tables S8–S9, Fig S4). In rate space, **humans and agents show no detectable difference** (log k_on/k_off/K_D MWU p=0.84/0.47/0.51; Fig 12a), with formal equivalence established for k_on and only a ≥7-fold cohort difference excludable for k_off and K_D.

![Fig 12](../figures/fig12_kinetic_decomposition.png)

***Fig 12.*** *Kinetic decomposition of binding, K_D = k_off / k_on, across rate-eligible designs: K_D variation is dominated by k_off (k_off–K_D ρ=0.92), and humans and agents are indistinguishable in rate space.*


Our binder with the best measured affinity, **1.11 nM (design 17, SPR-kinetic K_D)**, is competitive with the strongest reported de novo binders across all targets and **≈72× tighter than the best prior de novo TREM2 binder** (Perera et al., 2026; Odesign2, 80 nM), although this comparison should be interpreted cautiously because it contrasts an SPR kinetic (K_D) with an MST equilibrium measurement having an approximately 5 nM detection floor; four of our 36 quantified binders fall below that range. The target constructs are nevertheless closely matched across the directly relevant datasets, all spanning TREM2 residues 19–174 (Fig S6). The ten tightest binders are shown as a 3D gallery in **Fig S7**. 

**Design 79 as an inadvertent positive control.** One P_kd binder warrants separate mention. Design 79 (BraiNSEY, human, rank #7, 21.66 nM; flagged in **Fig S7**) is — per the rules-compliance note (Methods) — a near-copy of the anti-TREM2 antibody scFv-4, carrying its exact HCDR3 and a 100%-identity IGKV1-39 light chain, and the Methods note accordingly questions its status as a de novo human design. Read *post hoc* (this was not a pre-registered control), that same fact makes it a check on assay validity: a sequence from a class independently known to bind TREM2 entered the pipeline unlabelled, went through the identical cell-free expression and BLI/SPR workflow as every other design, and registered as a binder. The assay detected a true positive of known provenance — evidence of **sensitivity, and nothing more**. It does not validate quantitative calibration, does not validate any negative result (a single positive says nothing about the false-negative rate), and does not corroborate any specific measured K_D, including the 1.11 nM headline. Its 21.66 nM (SPR, monovalent single chain) sits ≈20× weaker than the ≈1 nM reported for the avidity-bearing parental scFv-4 (also SPR; Szykowska et al., 2021; Fig S6) — directionally what a loss of avidity would predict, though the monomer-versus-oligomer construct difference means we read this only as a consistency check, not a calibration.

Reclassifying a design as a control obliges a sensitivity test. Excluding design 79 (n=35) leaves every headline conclusion intact: interface size (ρ=0.45), buried area (0.48), PRODIGY ΔG (0.50) and binder length (0.63) still clear FDR, and the Protenix ipSAE stays non-significant (0.26). The one quantity that moves is the one already flagged as boundary-fragile — the competition's Boltz-2 ipSAE, whose ρ is essentially unchanged (0.40) but whose FDR clearance flips (q 0.10→0.11) when this single design is dropped, exactly the sensitivity anticipated above. Design 79 is not influential within the human cohort (human-arm ρ shifts ≤0.04 on every headline metric). We do **not** recompute the community hit-rate here: design 79 is a scored binder in the human arm, so excluding it would shift that cohort-level number — which belongs to the whole-study paper, not to this section; we flag it instead as an open item for the paper team, with these sensitivity figures attached.
...
to a per-predictor file for the SI cross-predictor comparison (SI Tables S7–S9), which anchors every comparison to Protenix. The learned confidence scores in §3 and Fig 11 are likewise the **Protenix** values (ipSAE, ipTM, mean pLDDT), so §3's metric comparison is apples-to-apples with §2; the competition's own Boltz-2 ipSAE (the submitted top-100 selection metric) is reported alongside for context. SI Table S8 gives every score on every model, where the confidence scores' backbone dependence is explicit. The
