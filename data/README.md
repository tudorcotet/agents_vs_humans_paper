...
│                          (curated metadata + pb_* + tp_* + b2_*/px_*/chai_*/af2m_*/ef2_*
...
│   ├── boltz2/         141 × Boltz-2 complex CIFs     (24 MB)  ← Modal rerun
│   ├── protenix/       141 × Protenix-v2 complex CIFs (28 MB)  ← Modal rerun
│   ├── chai/           141 × Chai-1 complex CIFs      (23 MB)  ← Modal rerun
│   ├── af2m/           141 × AF2-Multimer complex PDBs(22 MB)  ← Modal rerun
│   ├── esmfold2/       100 × ESMFold2 complex CIFs    (16 MB)  ← Modal rerun (100/141)
...
│   (the per-model scalar JSONs — boltz2/, af2m/, esmfold2/, prodigy_*/,
│    destress_* — are gitignored build inputs, regenerated by the Modal
│    scripts; only their scalars ship, in designs.csv)
...
mise run rerun:complex                     # boltz2 + protenix + chai1 + af2m + esmfold2
...
one row per design × every metric (`b2_*`, `px_*`, `chai_*`, `af2m_*`, `ef2_*`, `tp_*`)
